...
/* Fxcn port-S Wins, 0717-09 */
static char tolower(char c);
/* Fxcn port-E Wins, 0717-09 */
...
|\\xfe.?.?.?.?.?.?.?.?\\xfe|\\xff.?.?.?.?.?.?.?.?\\xff)"},
    {"netgeareva", L7_ENUM_NETGEAREVA, 0, "^...\\x53\\x4a\\x61\\x6d"}, /*foxconn add by pingod, 07/12/2008*/
...
/* Foxconn added start pling 11/08/2007 */
...
static int qos_l7_apps_priority[L7_ENUM_TOTAL + 1]; /* Apps enum starts from 1 to L7_ENUM_TOTAL, 0 is unused */
...
static void load_apps_priority(void)
{
    ...
    strcpy(qos_l7_appls, nvram_safe_get("qos_l7_apps"));
...
/* Foxconn added end pling 11/08/2007 */
...
static int __init init(void)
{
    ...
    load_apps_priority();       /* Foxconn added pling 11/08/2007 */
