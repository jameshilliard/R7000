...
/* Foxconn, Michael */
//for restrict the max number of media files. 
...
ScanDirectory(const char * dir, const char * parent, enum media_types dir_type)
{
	...
	if( chdir(dir) != 0 )
		return;
    /* Foxconn, Michael, 
     * should we restrict the scanned file number with limits?
     */
    //printf("%s: %d\n", __FUNCTION__, fileno);
	...
	for (i=0; i < n; i++)
	{
		...
		else if( type == TYPE_FILE )
		{
            ...
            {
				fileno++;
                /* Foxconn, Michael, should we restrict the scanned file number with limits? */
	...
	free(namelist);
	if( parent ) /*Foxconn, Michael, recursively called.*/
	{
	...
	else
	{
        ...
        printf("Scanning %s finished (%llu files)!\n",  dir, fileno);
	}
}
