...
/* foxconn wklin modified start, 10/13/2010, changes for workaround for sendfile() */
void
send_file(struct upnphttp * h, int sendfd, off_t offset, off_t end_offset)
{
	off_t send_size;
	off_t ret = -1;
    off_t lseek_offset = offset;
	...
	while( offset < end_offset )
	{
		...
		if( ret == -1 )
		{
            /* foxconn wklin added start, 10/08/2010 */
            /* filesystem has no sendfile() support (e.g. ntfs-3g with direct-io
             * enabled). Try slower way */
#define MAX_READ 256*1024
            ...
            int err = 0;
            n = send_size;
            if (lseek_offset>=0) { /* first time coming here */
                if (lseek_offset != offset)  {
                    /* this happens when sendfile support but fails
                     * to send after several rounds */
                    lseek(sendfd, offset, SEEK_SET);
                } else {
                    /* no sendfile support */
                    lseek(sendfd, lseek_offset, SEEK_SET);
                }
                /* mark this so we won't come here again */
                lseek_offset = -1;
            }
            while (n > 0 && !err) {
                if ((i = read(sendfd, buffer, (n>MAX_READ)?MAX_READ:n)) <= 0) {
                    /* either error (<0) or done (==0) */
                    if (i < 0)
                        err = 1;
                    else
			            DPRINTF(E_DEBUG, L_HTTP, "i=0\n");
                    break;
                }
                n -= i;
                p = &buffer[0];
                while (i > 0) {
                    if ((j = send(h->socket, p, i, 0)) < 0) {
                        /* something wrong */
                        err = 1;
                        break;
                    }
            ...
            /* foxconn wklin added end, 10/08/2010 */
		}
        /*
		else
		{
			fprintf(stdout, "ret != -1, sent %lld bytes to %d. offset is now %lld.\n", ret, h->socket, offset);
			DPRINTF(E_DEBUG, L_HTTP, "sent %lld bytes to %d. offset is now %lld.\n", ret, h->socket, offset);
		}
        */
	}
}
/* foxconn modified end, wklin, 10/13/2010 */
...
SendResp_resizedimg(struct upnphttp * h, char * object)
{
	...
	DPRINTF(E_WARN, L_HTTP, "	srcw:%d, srch:%d, dstw:%d,dsth:%d \n", srcw, srch, dstw, dsth);
...
SendResp_dlnafile(struct upnphttp * h, char * object)
{
	...
	if( newpid )
		return;
#if 0
	{/* Foxconn, add by MJ., 2010.12.28 */
		extern int sudp, shttpl;
