...
/* Foxconn, by MJ. */
#define ID_SCANNER 1
#define ID_MAINTHREAD 2
#define ID_SIZE_16M 3
...
open_db(int id)
{
    ...
    /* Foxconn, modified by Michael, 2011.08.08 */
    //sql_exec(db, "pragma  max_page_count = 256;"); /* The MAX of file.db will be under 32M. */
    if(id == ID_MAINTHREAD){
#if defined(WNDR4500REV) || defined(R4500)
        /* 16M */
	    sql_exec(db, "pragma default_cache_size = 4096;");
#else   /* 2M */
        /* Foxconn modified start pling 09/09/2011 */
        /* Change 512->256, otherwise device will be easy to get out of memory */
        sql_exec(db, "pragma default_cache_size = 256;");
        /* Foxconn modified end pling 09/09/2011 */
#endif
    }
    else if(id == ID_SIZE_16M) /* 16M */
        sql_exec(db, "pragma default_cache_size = 4096;");
    else if(id == ID_SCANNER) /* 1M */
        sql_exec(db, "pragma default_cache_size = 256;");
    else /* 1M */
        sql_exec(db, "pragma default_cache_size = 256;");
    /* Foxconn, modified-end by Michael. */
	return new_db;
}
...
main(int argc, char * * argv)
{
    ...
    /* Foxconn, Michael */
    int pid  = 0;
    int prio = -20; // I give minidlna the highest priority.
	...
	/*Foxconn modify start by Hank  12/08/2011*/
	/*dismark SendSSDPGoodbye because dlna will fail with no byebye message back
	  test upnp must kill minidlna process*/
	/* foxconn removed, zacker, 07/05/2011 */
	/* Don't send ssdp:byebye notifications at startup or else
	 * UPnP certification test case 2.0.0.0 and 2.0.0.1 will fail
	 */
	SendSSDPGoodbye(snotify, n_lan_addr);
	/*Foxconn modify end by Hank  12/08/2011*/
	...
	while(!quitting)
	{
		...
		if( scanning )
		{
			...
			if( !scanner_pid || kill(scanner_pid, 0) )
				scanning = 0;
            /* Foxconn, MJ: take a test */
            //waitpid(scanner_pid, &status, 0);

		}
        /* Foxconn, add by MJ., for debugging only. */
        #ifdef DEBUG
        printf(" ======== Goodbye ========.\n");
        #endif
        /* Foxconn, end by MJ. */
