...
static unsigned int host_tag = 0x000000f6;

#define DEBUG
#ifdef DEBUG
void ab_pppoe_log(char *msg)
{
    FILE *fp = fopen("/tmp/ppp/debug-log", "a");
...
int send_disc(struct session *ses, struct pppoe_packet *p)
{
    ...
    /* Foxconn modified start pling 02/22/2006 */
    /* Use static tag */
#if 0
	memcpy(tag->tag_data,
	       &ses,
	       sizeof(struct session *));
#endif
    memcpy(tag->tag_data, (char *)&host_tag, sizeof(host_tag));
    /* Foxconn modified end pling 02/22/2006 */
...
#define CHECK_TAG(name, val)					\
    ...
    /* Foxconn modified start pling 02/21/2006 */
    /* This MACRO seems to have alignment issue in some cases */
#if 1
    hu_val = *TAG_DATA(struct session* ,p->tags[TAG_HOST_UNIQ]);
#else
    {
        unsigned char *host_tag_addr;
        unsigned int  host_tag_data;
        int i;        
    
        host_tag_addr = (unsigned char *)(p->tags[TAG_HOST_UNIQ]->tag_data);

        for (host_tag_data=0, i=0; i<p->tags[TAG_HOST_UNIQ]->tag_len; i++)
        {
            host_tag_data <<= 8;
            host_tag_data += host_tag_addr[i];
        }
    
       hu_val = (struct session *)host_tag_data;
    }
#endif
    /* Foxconn modified end pling 02/21/2006 */

    /* Foxconn modified start pling 02/22/2006 */
    /* Compare static tag */
    /* if( hu_val != ses ){ */
    if( hu_val != (struct session *)host_tag ){
    /* Foxconn modified end pling 02/22/2006 */
...
/* Foxconn added start pling 02/22/2006 */
/* APIs used to close previous PPPoE connection upon startup */
struct pppoe_conn_info
{
    unsigned int  session_id;    
    unsigned char server_mac[6];
};

static struct pppoe_conn_info pppoe_prev_conn;
static int    send_padt = 0;

void pppoe_disconnect_prev_session(unsigned char *addr, unsigned int sid)
{
    send_padt = 1;
    pppoe_prev_conn.session_id  = sid;
    memcpy(pppoe_prev_conn.server_mac, addr, 6);
}

int session_disconnect_prev(struct session *ses)
{
    ...
    if (send_padt)
    {
        ...
        /* Store remote addr so that we can revert it later */
        memcpy(&remote_addr, &ses->remote, sizeof(struct sockaddr_ll));

        /* setup remote address */
        ses->remote.sll_family   = AF_PACKET;
        ses->remote.sll_protocol = htons(ETH_P_PPPOE_DISC); /* Foxconn modified pling 2006/12/13 */
        ses->remote.sll_hatype   = ARPHRD_ETHER;
        ses->remote.sll_pkttype  = 0;
        ses->remote.sll_halen    = 6;
        memcpy(ses->remote.sll_addr, pppoe_prev_conn.server_mac, 6);        

        /* setup session id */
        ses->sp.sa_addr.pppoe.sid = (unsigned short)(pppoe_prev_conn.session_id);        

        /* Sleep some time until WAN interface is ready to send */
        sleep(10);  /* Foxconn modified pling 2006/12/13 5->10 */
        session_disconnect(ses);
 
        /* Restore remote addr */
        memcpy(&ses->remote, &remote_addr, sizeof(struct sockaddr_ll));
...
again:
	...
	case PADI_CODE:
	{
        ...
        /* Foxconn added start pling 06/23/2006 */
        /* If we recv PADI from other devices, 
         * we just ignore it and recv the next packet.
         */
        else
            goto again;
        /* Foxconn added end pling 06/23/2006 */
	    break;
	}
	...
	case PADS_CODE:		/* wait for PADS */
	{
        ...
        sprintf(server_mac, "%02X%02X%02X%02X%02X%02X", 
        ...
        sprintf(session_id, "%d", rcv_packet.hdr->sid);
        script_setenv("SERVER_MAC", server_mac, 0);
        script_setenv("SESSION_ID", session_id, 0);
        /* Foxconn added end pling 02/22/2006 */
