...
static __u32 transmit_timestamp[2]; /* Foxconn added pling 10/31/2006 */
...
#define PREC -6
    ...
    /* Foxconn added start pling 10/31/2006 */
    /* Save the transmit timestamp */
    transmit_timestamp[0] = ntohl(data[10]);
    transmit_timestamp[1] = ntohl(data[11]);
    /* Foxconn added end pling 10/31/2006 */
...
#undef Data
    ...
    /* Foxconn added start pling 10/31/2006 */
    /* Add validity checks for reply packet */
    if (stratum == 0) {
        if (debug)
            printf("Got Kiss of death packet from NTP server.\n");
        exit(2);
    }

    if (orgtime.coarse != transmit_timestamp[0] || 
        orgtime.fine != transmit_timestamp[1]) {
		if (debug)
            printf("Incorrect original timestamp. Drop NTP reply.\n");
        exit(1);
    }

    if (xmttime.coarse == 0 && xmttime.fine == 0) {
        if (debug)
            printf("Transmit timestamp is 0. Drop NTP reply.\n");
        exit(1);
    }

    if (mode != 4) {
        if (debug)
            printf("Incorrect mode (%d). Drop NTP reply.\n", mode);
        exit(1);
    }
    /* Foxconn added end pling 10/31/2006 */
