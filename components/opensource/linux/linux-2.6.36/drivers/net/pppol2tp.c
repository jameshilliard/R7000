                ...
                int total_len)
{
    ...
    if (skb) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,31)
        rt = (struct rtable *)skb_rtable(skb);
#else
    ...
    iphdr->saddr = inet->inet_saddr; /* Source IP address */
    iphdr->daddr = inet->inet_daddr; /* Destination IP address */
#else
    ...
    if (rt) {
        ...
        ip_select_ident((struct iphdr *)iphdr, NULL, sk);
    }
    /* Header checksum */
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(2,6,32)
    tmp_uh.source = inet->inet_sport;
    tmp_uh.dest = inet->inet_dport;
#else
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(2,6,32)
    uh->check = csum_tcpudp_magic(inet->inet_saddr, inet->inet_daddr, skb->len, 
...
static struct file *pppol2tp_fget(pid_t pid, unsigned int fd)
{
	...
	if (pid != 0) {
		...
		struct task_struct *tsk = pid_task(find_get_pid(pid), PIDTYPE_PID);
#else
...
#ifdef CONFIG_PROC_FS
#if (LINUX_VERSION_CODE > KERNEL_VERSION(2,6,24))
	pppol2tp_proc = create_proc_entry("pppol2tp", 0644, init_net.proc_net);
#else
	pppol2tp_proc = create_proc_entry("pppol2tp", 0644, proc_net);
#endif
	if (!pppol2tp_proc) {
		err = -ENOMEM;
